function extractGeminiText(d){return (d?.candidates?.[0]?.content?.parts||[]).map(p=>p.text||'').join('').trim()}
function extractQwenText(d){return String(d?.choices?.[0]?.message?.content||'').trim()}
...
try{let g=await callQwen(prompt);let o=null;if(g.ok){try{o=parse(extractQwenText(g.data))}catch(e){o=null}if(o){const places=normalizePlaces(o);return res.json({reply:String(o.reply||'He actualizado la propuesta.'),places,source:g.provider,model:g.model})}}
 const key=String(process.env.GEMINI_API_KEY||'').trim();if(!key)return res.status(502).json({error:g.missing?'Falta configurar QWEN_API_KEY en Vercel':'Qwen no respondió y no hay GEMINI_API_KEY configurada'});g=await callGemini(key,{contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:.2,responseMimeType:'application/json'}});if(!g.ok)return res.status(g.status).json({error:g.data?.error?.message||'Error de IA'});o=parse(extractGeminiText(g.data));const places=normalizePlaces(o);return res.json({reply:String(o.reply||'He actualizado la propuesta.'),places,source:g.provider,model:g.model})}catch(e){return res.status(502).json({error:e.message||'Error'})}};
