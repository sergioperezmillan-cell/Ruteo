...
$('#optimize').onclick=async()=>{
 ...
 else{optimized=nearest(route,route[0]);optimized.startType='first'}
 $('#routeList').innerHTML='<h3>Tu recorrido</h3>'+optimized.map((p,i)=>`<div class="routeItem"><div class="num">${i+1}</div><div><b>${esc(p.name)}</b><div class="meta">${esc(p.type)}</div></div></div>`).join(''); $('#routeActions').classList.remove('hidden');window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'});
};
...
$('#maps').onclick=()=>{if(!optimized.length)return;let pts=optimized.map(p=>`${p.lat},${p.lon}`), start=$('input[name=start]:checked').value;let url='https://www.google.com/maps/dir/?api=1&travelmode='+mode;if(start==='current')url+='&origin=My+Location&destination='+encodeURIComponent(pts[pts.length-1]);else if(start==='custom')url+='&origin='+encodeURIComponent(`${optimized.startPoint?.lat},${optimized.startPoint?.lon}`)+'&destination='+encodeURIComponent(pts[pts.length-1]);else url+='&origin='+encodeURIComponent(pts[0])+'&destination='+encodeURIComponent(pts[pts.length-1]);let wp=start==='first'?pts.slice(1,-1):pts.slice(0,-1);if(wp.length)url+='&waypoints='+encodeURIComponent(wp.join('|'));window.open(url,'_blank')};
